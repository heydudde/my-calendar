...
A client-facing booking app that checks Google Calendar availability in real time and confirms or rejects appointment requests — no double booking, ever.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "36569521+heydudde@users.noreply.github.com"
  git config user.name "heydudde"
  ```
